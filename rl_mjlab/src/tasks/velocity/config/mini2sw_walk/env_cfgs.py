"""Mini2SW walking velocity environment configurations."""

from src.assets.robots import get_mini2sw_walk_robot_cfg
...
def _mini2sw_walk_base_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create Mini2SW base walking velocity configuration with wheel foot contact."""
  ...
  cfg.scene.entities = {
    "robot": with_delayed_articulation(get_mini2sw_walk_robot_cfg(), play=play),
  }

  foot_names = ("fl", "fr", "bl", "br")
  site_names = foot_names
  wheel_geom_names = tuple(f"{name}_wheel_collision" for name in foot_names)
...
def mini2sw_flat_walk_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create Mini2SW flat terrain walking velocity configuration."""
  cfg = _mini2sw_walk_base_env_cfg(play=play)
